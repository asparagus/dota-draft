...
class Model(pl.LightningModule):
    ...
    def forward(self, x):
        team = self.embeddings(x)
        radiant = team.narrow(1, 0, 5).sum(dim=1, keepdim=False)
        dire = team.narrow(1, 5, 5).sum(dim=1, keepdim=False)

        radiant_combined = torch.cat([radiant, dire], dim=1)
        dire_combined = torch.cat([dire, radiant], dim=1)
        ...
        radiant_final = self.final_layer(radiant_combined)
        dire_final = self.final_layer(dire_combined)
...
if __name__ == '__main__':
    ...
    wr_ingester = data_ingestion.WinRateIngester(single_heroes=True, hero_pairs=True, hero_matchups=True)
